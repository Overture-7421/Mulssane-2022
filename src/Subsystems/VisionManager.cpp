...
VisionManager::VisionManager(Chassis* chassis) : visionNotifier([&]{
    photonlib::PhotonPipelineResult result = camera.GetLatestResult();
    if(result != lastResult){
        updateCircleFit(result);
        units::second_t targetTimestamp = frc::Timer::GetFPGATimestamp() - result.GetLatency();
        chassis->addVisionMeasurement(visionPose, targetTimestamp.value());
        lastResult = result;
    }
}){
...
void VisionManager::updateCircleFit(const photonlib::PhotonPipelineResult& result){
    if(!result.HasTargets() || (int) result.GetTargets().size() < minTargetCount || camera.GetLEDMode() != photonlib::LEDMode::kOn) return;

    const auto targets = result.GetTargets();
    std::vector<frc::Translation2d> outputTranslations;

    std::for_each(targets.begin(), targets.end(), [&](const photonlib::PhotonTrackedTarget& target){
    ...
    frc::Rotation2d cameraRotation = robotRotation.RotateBy(CameraConstants::cameraToRobot.Rotation());
    frc::Transform2d fieldToTargetRotated {fieldToTarget.Translation(), cameraRotation};

    frc::Transform2d fieldToCamera = fieldToTargetRotated + frc::Transform2d(-circleTranslation, {});
    frc::Transform2d fieldToRobot = fieldToCamera + CameraConstants::cameraToRobot.Inverse();
    visionPose = {fieldToRobot.Translation(), fieldToRobot.Rotation()};
}

std::vector<std::pair<double, double>> VisionManager::sortPoints(const std::vector<std::pair<double, double>>& points){

    //Get sum of all points
    auto targetCenter = std::accumulate(points.begin(), points.end(),
    ...
    for (int i = 0; i < (int) points.size(); i++) {
      ...
      double angleRad =
          (frc::Rotation2d(corner.first - targetCenter.first, targetCenter.second - corner.second) - frc::Rotation2d(90_deg)).Radians().value();
      if (angleRad > 0) {
...
std::optional<frc::Translation2d> VisionManager::cameraToTargetTranslation(std::pair<double, double> corner, const units::meter_t targetHeight){
    double yPixels = corner.first;
    double zPixels = corner.second;

    // Robot frame of reference
    double nY = -((yPixels - (CameraConstants::resolution.first / 2)) / (CameraConstants::resolution.first / 2));
    double nZ = -((zPixels - (CameraConstants::resolution.second / 2)) / (CameraConstants::resolution.second / 2));

    frc::Translation2d xzPlaneTranslation =
        frc::Translation2d(1.0_m, units::meter_t(CameraConstants::vph / 2.0 * nZ)).RotateBy(CameraConstants::pitch);
    double x = xzPlaneTranslation.X().value();
    double y = CameraConstants::vpw / 2.0 * nY;
    double z = xzPlaneTranslation.Y().value();

    double differentialHeight = (CameraConstants::height - this->targetHeight).value();
...
std::optional<VisionManager::Circle> VisionManager::solveLeastSquaresCircle(const std::vector<frc::Translation2d>& circlePoints){
    std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> eigenPoints;
    Eigen::Vector2d midpoint;
    Circle circle;

    std::transform(circlePoints.begin(), circlePoints.end(), std::back_inserter(eigenPoints),
    ...
    circle.midpoint = frc::Translation2d(units::meter_t(midpoint.x()), units::meter_t(midpoint.y()));
